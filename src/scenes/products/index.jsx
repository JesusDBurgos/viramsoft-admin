...
import { FormControl, InputLabel, Card, CardMedia } from "@mui/material";
...
const Products = () => {
        ...
        <Box m="20px">
          ...
          <Formik
            initialValues={{
              ...
              imagen:
                selectedProduct && selectedProduct.imagenes
            ...
            onSubmit={async (values, { resetForm }) => {
              try {
                const response = await fetch(
                  `https://viramsoftapi.onrender.com/edit_product/${selectedProduct.idProducto}`,
                  {
                    method: "PUT",
                    headers: {
                ...
                if (error.status === 404) {
                  ...
                  setMensaje("Error inesperado editando el producto");
                }
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (imagenSeleccionada) {
                          ...
                          fetch(
                            `https://viramsoftapi.onrender.com/cargar_imagen?producto_id=${selectedProduct.idProducto}`,
                            {
                              method: "POST",
                              body: formData,
                            }
  ...
  const checkoutSchema = yup.object().shape({
    ...
    marca: yup
      .string()
      .required("Requerido")
      .max(20, "La marca debe tener como máximo 20 caracteres"),
    ...
    unidadMedida: yup
      .string()
      .required("Requerido")
      .max(10, "La unidad de medida debe tener como máximo 10 caracteres"),
  });
  ...
  const OpenAddProductoDialog = () => {
    ...
    const handleFormSubmit = async (values) => {
      try {
        ...
        const response = await fetch(
          "https://viramsoftapi.onrender.com/create_product",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <Box m="20px" mt="-50px">
      ...
      >
        ...
        <DataGrid rows={productosData} columns={columns} />
      </Box>
